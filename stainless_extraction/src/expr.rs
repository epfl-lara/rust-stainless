...
impl<'a, 'l, 'tcx> BodyExtractor<'a, 'l, 'tcx> {
  ...
  fn extract_unary(&mut self, expr: &Expr<'a, 'tcx>) -> st::Expr<'l> {
    let f = self.factory();
  ...
  fn extract_binary(&mut self, expr: &Expr<'a, 'tcx>) -> st::Expr<'l> {
    let f = self.factory();
  ...
  fn extract_logical_op(&mut self, expr: &Expr<'a, 'tcx>) -> st::Expr<'l> {
    let f = self.factory();
